...
class Products extends Model {
	...
	public function add($id_category,$id_brand,$name, $description,$stock,$price_from,$price, $weight,$width,$height,$length,$diameter,$featured , $sale,$bestseller,$new_product, $options) {

		if (!empty($id_category) && !empty($id_brand) && !empty($name) && !empty($stock) && !empty($price)) {

			$options_salected = array();

			foreach($options as $optk => $opt) {
				if(!empty($opt)) {
					$options_salected[$optk] = $opt;					
				}
			}

			$options_ids =implode(',', array_keys($options_salected));
			...
			foreach($options_salected as $optk => $opt) {
